...
import ec.edu.utpl.adopcionmascotas.modelo.pojo.Rol;
...
public class GestionRol extends javax.swing.JFrame {
    ...
    private static final String CLASS_NAME = GestionRol.class.getName();
    
    
    public GestionRol(Sesion sesion, Integer crol, String cambio) {

        try {
            this.sesion = sesion;
            this.validacion = new Validacion();
            this.rol = new Rol(sesion.getIdSesion());
            this.crol = crol;
            this.cambio = cambio;
    ...
    private void setTitulos(){
       if(ACCION_NUEVO.equals(cambio)) {
           lblSubtituloEmpresa.setText("Creación de Rol en el Sistema");
           lblDato.setText("Nuevo Rol");
           lblInfo.setText("Ingrese la información del nuevo rol:");
           btnAceptarEmpresa.setText("CREAR");
       } else if(ACCION_EDITAR.equals(cambio)) {
           lblSubtituloEmpresa.setText("Editar Rol del Sistema");
           lblDato.setText(rol.getDescripcion());
           ...
           lblSubtituloEmpresa.setText("Eliminar Rol del Sistema");
           lblDato.setText(rol.getDescripcion());
           lblInfo.setText("Información del rol que se desea eliminar:");
    ...
    private void loadRol(){
        ...
        catalogo.setComboCatalogo("ACTIVO", null, cmbEstado);
    ...
    private void initComponents() {
        ...
        lblSubtituloEmpresa.setText("Creación de Roles en el Sistema");
        ...
        lblElementos.setText("Módulos Asignados:");
        ...
        panCentral.add(scrLista, new org.netbeans.lib.awtextra.AbsoluteConstraints(150, 230, 150, 170));
    ...
    private void btnAceptarEmpresaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAceptarEmpresaActionPerformed
       if(ACCION_NUEVO.equals(cambio)) {
           setInfoRol();
           rol.newRol();
           JOptionPane.showMessageDialog(rootPane,"Se ha creado correctamente el rol.","Roles del Sistema",JOptionPane.INFORMATION_MESSAGE);
           volver(sesion);
       } else if(ACCION_EDITAR.equals(cambio)) {
           setInfoRol();
           rol.updateRol();
           JOptionPane.showMessageDialog(rootPane,"Se ha actualizado correctamente el rol.","Roles del Sistema",JOptionPane.INFORMATION_MESSAGE);
           volver(sesion);
       } else if(ACCION_ELIMINAR.equals(cambio)) {
           if("administrador".equals(rol.getNombre())){
               JOptionPane.showMessageDialog(rootPane,"No se puede eliminar el rol administrador por defecto.","Roles del Sistema",JOptionPane.ERROR_MESSAGE);
           } else {
                rol.deleteRol();
                JOptionPane.showMessageDialog(rootPane,"Se ha eliminado correctamente el rol.","Roles del Sistema",JOptionPane.INFORMATION_MESSAGE);
                volver(sesion);
           }   
       }
    ...
    private void setInfoRol(){
        rol.setNombre(txtNombre.getText());
        rol.setDescripcion(txtDescripcion.getText());
        rol.setEstado(cmbEstado.getSelectedItem().toString());

    }
    
    private void getInfoRol(){
        if(this.rol != null){
            rol.getRol(this.crol);
            txtCodigo.setText(this.crol.toString());
            txtNombre.setText(rol.getNombre());
            txtDescripcion.setText(rol.getDescripcion());
            cmbEstado.setSelectedItem(rol.getEstado());
        } 
    }
