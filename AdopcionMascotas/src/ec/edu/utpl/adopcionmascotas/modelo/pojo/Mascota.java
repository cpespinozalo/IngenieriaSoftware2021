...
public class Mascota implements Serializable{
    ...
    private static final String SQL_LISTADOIMAGENESMASCOTAS = "SELECT RUTA FROM TIMAGENMASCOTA WHERE CMASCOTA=? ";
    private static final String SQL_INSERTIMAGENMASCOTA = "INSERT INTO TIMAGENMASCOTA (CIMAGEN,RUTA,CMASCOTA) VALUES (?,?,?)";
    private static final String SQL_CIMAGEN = "SELECT NVL(MAX(CIMAGEN),1000) FROM TIMAGENMASCOTA";
    private static final String SQL_CMASCOTA = "SELECT CMASCOTA FROM TMASCOTA WHERE NOMBRE=? AND CUSUARIOPUBLICACION=(SELECT CUSUARIO FROM TUSUARIO WHERE USUARIO=?) ";
    ...
    public List<String> getImagenesMascota(Integer cmascota){
        
        List<String> listaImagenes = new ArrayList<>();
        Cliente cliente = new Cliente();
        List<Object> resultado = resultado = cliente.query(SQL_LISTADOIMAGENESMASCOTAS, cmascota);       
        for(Object dato : resultado){
            listaImagenes.add(dato.toString());
        }
        return listaImagenes;
    }
    
    public Integer getCmascotanuevo(){
        
        Cliente cliente = new Cliente();
        return cliente.queryUnique(SQL_CMASCOTA, nombremascota, cusuariopublicacion);
    }
    
    public Integer getCimagen(){
        
        Cliente cliente = new Cliente();
        return cliente.queryUnique(SQL_CIMAGEN);
    }
    ...
    public boolean newImagenMascota(Integer cimagen, Integer cmascota, String archivo){
        
        String tipo= "MODIFICACION";
        String accion= "CAMBIOS EN EL SISTEMA";
        String detalle = "GUARDADO FOTO MASCOTA";
        
        Cliente cliente = new Cliente();
        int registros = cliente.execute(SQL_INSERTIMAGENMASCOTA, cimagen, archivo, cmascota);
        if(registros > 0){
            auditoria.registrarAuditoria(sesionId, tipo, accion, detalle, "OK");
            return true;
        } else {
