...
public class Mascota implements Serializable{
    ...
    private static final String SQL_SELECT = "SELECT MAS.CMASCOTA,MAS.NOMBRE,(SELECT NOMBRE FROM TITEM WHERE CODIGO=MAS.TIPOMASCOTA AND CCATALOGO=5007),MAS.EDAD,(SELECT NOMBRE FROM TITEM WHERE CODIGO=MAS.UNIDADEDAD AND CCATALOGO=5001) UNIDADEDAD,(SELECT NOMBRE FROM TITEM WHERE CODIGO=MAS.GENERO AND CCATALOGO=5008) GENERO,MAS.DESCRIPCION,(SELECT USUARIO FROM TUSUARIO WHERE CUSUARIO=MAS.CUSUARIOPUBLICACION) CUSUARIOPUBLICACION,TO_CHAR(MAS.FPUBLICACION,'dd-mm-yyyy') FPUBLICACION,(SELECT NOMBRE FROM TITEM WHERE CODIGO=MAS.ESTADO AND CCATALOGO=5006) ESTADO,USU.NOMBRES || ' ' || USU.APELLIDOS PROPIETARIO,USU.DIRECCION,(SELECT NOMBRE FROM TITEM WHERE CODIGO=USU.PROVINCIA AND CCATALOGO=5002) PROVINCIA,(SELECT NOMBRE FROM TITEM WHERE CODIGO=USU.CIUDAD AND CODIGOPADRE=USU.PROVINCIA AND CCATALOGO=5003) CIUDAD,USU.TELEFONO,USU.CORREO,(SELECT TO_CHAR(FADOPCION,'dd-mm-yyyy') FROM TADOPCION WHERE CMASCOTA=MAS.CMASCOTA AND ESTADOADOPCION IN ('ING','PRO') AND ACTIVO=1) FADOPCION, NVL((SELECT (SELECT NOMBRE FROM TITEM WHERE CODIGO=ADO.ESTADOADOPCION AND CCATALOGO=5009) ESTADOADOPCION FROM TADOPCION ADO WHERE ADO.CMASCOTA=MAS.CMASCOTA AND ADO.ACTIVO=1),'NOA') ESTADOADOPCION FROM TMASCOTA MAS, TUSUARIO USU WHERE MAS.CUSUARIOPUBLICACION=USU.CUSUARIO AND MAS.CMASCOTA=? ";
